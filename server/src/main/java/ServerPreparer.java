...
public class ServerPreparer {
    private static boolean graphicMode = false;
    ...
    private static String dbType = "",
            ...
            dbUserName = "",
            dbPassword = "";
    private static StringBuilder dbConnectionString = new StringBuilder("jdbc:");      // сюда собираем строку для подключения к базе
    private static String mysql = "mysql",
            postgresql = "postgresql";
    private static final String noSSL = "?useSSL=false";     // исключаем предупреждение из логов; будет добавлено к строке подключения к БД последним
    private static String driverClassName = "";        // сюда положим нужный драйвер в зависимости от типа БД

    /**
     * usage:
     * no args or -c key => run basic configuration of server settings and database
     * full_path_to_settings_file => where to store settings
     * -cg => configure server settings and database running GUI
     * */
    public static void main(String[] args) {
        if (args.length > 0 && "-cg".equals(args[0]))
            graphicMode = true;
        else consoleReader = new BufferedReader(new InputStreamReader(System.in));

        if (args.length > 0 && args[0].charAt(0) != '-') {
            settingsPath = args[0];
        } else settingsPath = "./settings";

        if (graphicMode) runGUI();
        else {
            try {
    ...
    /**
     * Saving class fields values into settings file
     * */
